...
const defaultOptions: LayoutOptions = {
  title: "Tiago Luchini",
  description:
    "CTO, Digital Product Innovation Strategist, Business Leader, and Engineer",
...
// escapeHtml (declared below, hoisted) sanitizes every frontmatter-derived
...
<body class="min-h-screen bg-base-100 text-base-content" hx-boost="true">
  ...
  <script>
    ...
    function updateActiveNavLink() {
      ...
      const navLinks = document.querySelectorAll('nav a[href], .drawer-side a[href]');
...
const socialLinks = [
  {
    href: "https://twitter.com/tiagoluchini",
    label: "Twitter",
  ...
  {
    href: "https://github.com/luchiniatwork",
    label: "GitHub",
  ...
  {
    href: "https://linkedin.com/in/tiagoluchini",
    label: "LinkedIn",
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>`,
  },
  {
    href: "mailto:tiago@luchini.nyc",
    label: "Email",
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M0 3v18h24v-18h-24zm21.518 2l-9.518 7.713-9.518-7.713h19.036zm-19.518 14v-11.817l10 8.104 10-8.104v11.817h-20z"/></svg>`,
  },
];

/** Hamburger menu icon */
const hamburgerIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" /></svg>`;
    ...
    <header class="sticky top-0 z-40 bg-base-100 shadow-sm">
      <div class="container mx-auto px-4 max-w-4xl py-4">
        <div class="flex items-center justify-between">
          ...
          <label for="nav-drawer" class="btn btn-ghost btn-square sm:hidden" aria-label="Open menu">
            ${hamburgerIcon}
          </label>
 ...
 * Mobile drawer sidebar
 */
export function mobileDrawer(currentPath: string = "/"): string {
  const isActive = (href: string) => {
    if (href === "/") return currentPath === "/";
    return currentPath?.startsWith(href);
  };
  ...
  return `
    <div class="drawer-side z-50">
      ...
      <div class="menu bg-base-200 min-h-full w-64 p-4">
        <!-- Drawer header -->
        <div class="mb-4 pb-4 border-b border-base-300">
          <span class="text-lg font-bold">Tiago Luchini</span>
        </div>
        ...
        <ul class="space-y-1">
          ${navLinks
            ...
            <li>
              <a href="${link.href}" 
                 class="btn btn-ghost justify-start ${isActive(link.href) ? "btn-active text-secondary" : ""}"
                 hx-indicator="#loading-indicator"
                 hx-target="#main-content"
                 hx-select="main#main-content > *"
                 hx-swap="innerHTML show:window:top">
        ...
        <div class="mt-auto pt-4 border-t border-base-300">
          <p class="text-sm opacity-60 mb-2">Connect</p>
 ...
 */
export function nav(_currentPath: string = "/"): string {
